...
type (
	...
	ShowOrdersCommand struct {
		// End Order date(UnixTime)
		TimeEnd int
		// Start Order date(UnixTime)
		TimeStart int
		// Unique user ID
		User        int
		PrettyPrint bool
...
func (cmd *ShowOrdersCommand) Run(c *client.Client, args []string) error {
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.ShowOrders(ctx, path, cmd.TimeEnd, cmd.TimeStart, intFlagVal("user", cmd.User))
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
func (cmd *ShowOrdersCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	cc.Flags().IntVar(&cmd.TimeEnd, "time_end", 2147483647, `End Order date(UnixTime)`)
	var timeStart int
	cc.Flags().IntVar(&cmd.TimeStart, "time_start", timeStart, `Start Order date(UnixTime)`)
	var user int
	cc.Flags().IntVar(&cmd.User, "user", user, `Unique user ID`)
}
