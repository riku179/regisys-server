...
type (
	// AddGoodsCommand is the command line data structure for the add action of goods
	AddGoodsCommand struct {
		Payload     string
		ContentType string
		PrettyPrint bool
	}

	// DeleteGoodsCommand is the command line data structure for the delete action of goods
	DeleteGoodsCommand struct {
		// Unique item ID
		ID          int
		PrettyPrint bool
	}

	// ModifyGoodsCommand is the command line data structure for the modify action of goods
	ModifyGoodsCommand struct {
		Payload     string
		ContentType string
		// Unique item ID
		ID          int
	...
	// ShowGoodsCommand is the command line data structure for the show action of goods
	ShowGoodsCommand struct {
		// User ID
		User        int
		PrettyPrint bool
	}
...
{
   "date": "2016-01-18T00:05:35+09:00",
   "item_id": 5152369185030202978,
   "price": 8146831818233394072,
   "quantity": 663468924056338539,
   "register": "Aut illo."
}`,
...

{
   ...
   "item": "Optio molestiae magni quae ut tenetur.",
...
// Run makes the HTTP request corresponding to the AddGoodsCommand command.
func (cmd *AddGoodsCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.AddGoods(ctx, path, &payload, cmd.ContentType)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
func (cmd *AddGoodsCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	cc.Flags().StringVar(&cmd.Payload, "payload", "", "Request body encoded in JSON")
	cc.Flags().StringVar(&cmd.ContentType, "content", "", "Request content type override, e.g. 'application/x-www-form-urlencoded'")
}

// Run makes the HTTP request corresponding to the DeleteGoodsCommand command.
func (cmd *DeleteGoodsCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
		path = fmt.Sprintf("/goods/%v", cmd.ID)
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.DeleteGoods(ctx, path)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *DeleteGoodsCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	var id int
	cc.Flags().IntVar(&cmd.ID, "id", id, `Unique item ID`)
}

// Run makes the HTTP request corresponding to the ModifyGoodsCommand command.
func (cmd *ModifyGoodsCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.ModifyGoods(ctx, path, &payload, cmd.ContentType)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *ModifyGoodsCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	cc.Flags().StringVar(&cmd.Payload, "payload", "", "Request body encoded in JSON")
	cc.Flags().StringVar(&cmd.ContentType, "content", "", "Request content type override, e.g. 'application/x-www-form-urlencoded'")
	var id int
	cc.Flags().IntVar(&cmd.ID, "id", id, `Unique item ID`)
...
// Run makes the HTTP request corresponding to the ShowGoodsCommand command.
func (cmd *ShowGoodsCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.ShowGoods(ctx, path, intFlagVal("user", cmd.User))
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *ShowGoodsCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	var user int
...
func (cmd *ModifyUserCommand) Run(c *client.Client, args []string) error {
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.ModifyUser(ctx, path, stringFlagVal("group", cmd.Group))
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
func (cmd *ModifyUserCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	...
	var group string
	cc.Flags().StringVar(&cmd.Group, "group", group, `Group of user`)
}
